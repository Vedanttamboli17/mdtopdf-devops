...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    h1 { font-family: var(--sans); font-size: clamp(2.4rem, 6vw, 4.2rem); font-weight: 800; line-height: 1.05; letter-spacing: -.02em; }
    h1 em { font-style: normal; color: var(--accent); }
    .sub { margin-top: 14px; color: var(--muted); font-size: 13px; letter-spacing: .04em; }
    .card { width: 100%; max-width: 680px; background: var(--surface); border: 1px solid var(--border); border-radius: 16px; padding: 36px; position: relative; overflow: hidden; }
    ...
    #drop-zone { border: 2px dashed var(--border); border-radius: 12px; padding: 48px 24px; display: flex; flex-direction: column; align-items: center; gap: 12px; cursor: pointer; transition: border-color .2s, background .2s; text-align: center; position: relative; }
    ...
    #file-chip { display: none; align-items: center; gap: 10px; background: rgba(0,229,160,.07); border: 1px solid rgba(0,229,160,.2); border-radius: 8px; padding: 10px 14px; font-size: 13px; margin-top: 16px; }
    ...
    #upload-btn { width: 100%; margin-top: 20px; padding: 14px; border: none; border-radius: 10px; background: var(--accent); color: #000; font-family: var(--sans); font-size: 15px; font-weight: 700; letter-spacing: .04em; cursor: pointer; transition: opacity .2s, transform .15s; display: flex; align-items: center; justify-content: center; gap: 8px; }
    ...
    #pipeline { display: none; margin-top: 28px; gap: 0; flex-direction: column; }
    #pipeline.show { display: flex; }
    .pipe-step { display: flex; align-items: flex-start; gap: 16px; position: relative; padding-bottom: 24px; }
    ...
    #download-block { display: none; margin-top: 28px; background: rgba(0,229,160,.06); border: 1px solid rgba(0,229,160,.2); border-radius: 12px; padding: 22px; flex-direction: column; align-items: center; gap: 12px; text-align: center; animation: fadeUp .4s ease; }
    @keyframes fadeUp { from{opacity:0;transform:translateY(10px)} to{opacity:1;transform:none} }
    #download-block.show { display: flex; }
    .dl-icon { font-size: 36px; animation: bounce .6s ease; }
    ...
    #error-banner { display: none; margin-top: 20px; background: rgba(255,69,96,.08); border: 1px solid rgba(255,69,96,.3); border-radius: 10px; padding: 14px 18px; font-size: 12px; color: var(--danger); gap: 8px; align-items: flex-start; }
    #error-banner.show { display: flex; }
    #rate-banner { display: none; margin-top: 20px; background: rgba(255,150,0,.08); border: 1px solid rgba(255,150,0,.3); border-radius: 10px; padding: 14px 18px; font-size: 12px; color: #ff9600; gap: 8px; align-items: flex-start; }
    ...
    summary { padding: 16px 20px; cursor: pointer; font-size: 12px; color: var(--muted); letter-spacing: .08em; text-transform: uppercase; list-style: none; display: flex; align-items: center; gap: 8px; user-select: none; transition: color .2s; }
    summary:hover { color: var(--text); }
    summary::after { content: '▾'; margin-left: auto; transition: transform .2s; }
    details[open] summary::after { transform: rotate(180deg); }
    .arch-body { padding: 0 20px 20px; display: flex; flex-direction: column; gap: 4px; }
    ...
    .arch-svc { color: var(--accent); font-weight: 600; min-width: 90px; }
...
<body>
  ...
  <div class="shell">
    <header>
      <div class="badge"><span></span> AWS Event-Driven System</div>
      <h1>Markdown <em>→</em> PDF</h1>
      <p class="sub">Distributed microservices · Go + Fiber · S3 · SQS · DynamoDB</p>
    </header>
    <div class="card">
      <div id="drop-zone">
        <input type="file" id="file-input" accept=".md" />
        <div class="drop-icon">📄</div>
        <div class="drop-title">Drop your Markdown file here</div>
        <div class="drop-sub">or click to browse</div>
        <div class="drop-meta">.md files only · max 5 MB</div>
      </div>
      <div id="file-chip">
        <span>📎</span>
        <span class="chip-name" id="chip-name">file.md</span>
      ...
      <div id="rate-banner">⏳ Rate limit hit — max 5 uploads/min. Please wait a moment.</div>
      <div id="pipeline">
        <div class="pipe-step" id="step-upload">
          ...
          <div class="step-body">
            ...
            <div class="step-desc">Sending your Markdown file to Amazon S3</div>
          </div>
        </div>
        <div class="pipe-step" id="step-queue">
          <div class="step-dot"><div class="dot-spinner"></div><div class="dot-icon">⬡</div></div>
          <div class="step-body">
            <div class="step-label">Queued in SQS</div>
            <div class="step-desc">Job message sent to Amazon SQS queue</div>
        ...
        <div class="pipe-step" id="step-worker">
          ...
          <div class="step-body">
            ...
            <div class="step-desc">Worker pulled job · converting MD → HTML → PDF</div>
          </div>
        </div>
        <div class="pipe-step" id="step-done">
          <div class="step-dot"><div class="dot-spinner"></div><div class="dot-icon">✓</div></div>
          <div class="step-body">
            <div class="step-label">PDF Ready</div>
            <div class="step-desc">PDF stored in S3 · presigned URL generated</div>
          </div>
        </div>
      </div>
      <div id="download-block">
        ...
        <div class="dl-sub">Presigned URL expires in 10 minutes</div>
    ...
    <div class="info-panel">
      <details>
        ...
        <div class="arch-body">
          <div class="arch-row"><div class="arch-num">1</div><div class="arch-svc">API Service</div><div class="arch-desc">Go + Fiber validates file, uploads to S3, creates DynamoDB job, sends SQS message</div></div>
          <div class="arch-row"><div class="arch-num">2</div><div class="arch-svc">Amazon S3</div><div class="arch-desc">Stores raw Markdown (uploads/) and generated PDFs (pdfs/) with private access</div></div>
          <div class="arch-row"><div class="arch-num">3</div><div class="arch-svc">DynamoDB</div><div class="arch-desc">Tracks job metadata: status, input_url, output_url, retry_count</div></div>
          <div class="arch-row"><div class="arch-num">4</div><div class="arch-svc">Amazon SQS</div><div class="arch-desc">Decouples API from Worker. Long-polling, DLQ after 3 retries</div></div>
          <div class="arch-row"><div class="arch-num">5</div><div class="arch-svc">Worker</div><div class="arch-desc">Polls SQS, downloads MD, converts via wkhtmltopdf, uploads PDF, updates DynamoDB</div></div>
          <div class="arch-row"><div class="arch-num">6</div><div class="arch-svc">Presigned URL</div><div class="arch-desc">S3 presigned URL gives temporary 10-min access. PDF never publicly exposed</div></div>
        </div>
      </details>
    </div>
    <footer>Built with <span>Go · Fiber · Docker · Terraform · Ansible · Jenkins</span> · AWS Free Tier</footer>
  </div>
  <script>
    ...
    const fileIdTag   = document.getElementById('file-id-tag');
    const steps = {
    ...
    chipRemove.addEventListener('click', e => { e.stopPropagation(); resetFile(); });

    function setFile(file) {
      if (!file) return;
      if (!file.name.endsWith('.md')) { showError('Only .md (Markdown) files are accepted.'); return; }
      if (file.size > 5 * 1024 * 1024) { showError('File too large — max 5 MB allowed.'); return; }
      selectedFile = file;
      chipName.textContent = file.name;
      fileChip.classList.add('show');
      uploadBtn.disabled = false;
      btnText.textContent = 'Convert "' + file.name + '" to PDF';
      hideError();
    }
    ...
    async function startUpload() {
      ...
      pipeline.classList.add('show'); setStep('upload', 'active');
    ...
    async function pollStatus() {
      ...
      try {
        ...
        if (data.status === 'completed') {
          ...
          setStep('worker', 'done'); setStep('done', 'done');
          showDownload(data.download_url);
        } else if (data.status === 'failed') {
          clearInterval(pollInterval);
          setStep('worker', 'failed'); setStep('done', 'failed');
          showError('Conversion failed after max retries. Check worker logs.');
          uploadBtn.disabled = false; btnText.textContent = 'Try Again';
        }
      } catch (err) { console.warn('Poll error:', err); }
    }

    function showDownload(url) {
      dlBtn.href = url; dlBlock.classList.add('show'); uploadBtn.style.display = 'none';
    }

    newBtn.addEventListener('click', () => {
      clearInterval(pollInterval); currentFileId = null;
      Object.values(steps).forEach(s => s.className = 'pipe-step');
      pipeline.classList.remove('show'); dlBlock.classList.remove('show');
    ...
    function resetToIdle() {
      ...
      btnText.textContent = selectedFile ? 'Convert "' + selectedFile.name + '" to PDF' : 'Select a file to convert';
